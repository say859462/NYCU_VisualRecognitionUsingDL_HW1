...
class CompactBilinearPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        sketch1 = torch.zeros(B, self.output_dim, H*W, device=x.device).scatter_add_(1, self.h1.view(1,C,1).expand(B,C,H*W), x1)
        sketch2 = torch.zeros(B, self.output_dim, H*W, device=x.device).scatter_add_(1, self.h2.view(1,C,1).expand(B,C,H*W), x2)
...
class ImageClassificationModel(nn.Module):
    def __init__(self, num_classes: int = 100, pretrained: bool = True):
        ...
        # C. 決策層
        self.embedding_cbp = nn.Sequential(nn.Linear(2048, 512), nn.BatchNorm1d(512), nn.PReLU(), nn.Dropout(0.5))
        self.classifier_cbp = NormedLinear(512, num_classes)
        self.embedding_gem = nn.Sequential(nn.Linear(1024, 512), nn.BatchNorm1d(512), nn.PReLU(), nn.Dropout(0.5))
    ...
    def _make_layer(self, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * ResNeXtifiedBottleneck.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * ResNeXtifiedBottleneck.expansion, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(planes * ResNeXtifiedBottleneck.expansion),
            )
        layers = []
        layers.append(ResNeXtifiedBottleneck(self.inplanes, planes, stride, downsample, self.groups, self.base_width))
        self.inplanes = planes * ResNeXtifiedBottleneck.expansion
        for _ in range(1, blocks):
            layers.append(ResNeXtifiedBottleneck(self.inplanes, planes, groups=self.groups, base_width=self.base_width))
        return nn.Sequential(*layers)

    def _load_official_weights(self):
        """
        核心技術：權重手術 (Weight Surgery)
        從 ResNet-152 提取權重並嫁接到手寫 ResNeXt 架構。
        """
        print("💉 Initiating Weight Surgery: ResNet-152 -> Custom ResNeXt...")
        official_resnet = models.resnet152(weights=models.ResNet152_Weights.DEFAULT).state_dict()
        model_dict = self.state_dict()

        # 修正後的映射邏輯：處理 backbone_l1_l3 前綴
        processed_dict = {}
        for k, v in official_resnet.items():
            ...
            # 2. 處理封裝在 backbone_l1_l3 內的層級 (layer1, layer2, layer3)
            l1_l3_key = f"backbone_l1_l3.{k}"
            if l1_l3_key in model_dict:
                if v.shape == model_dict[l1_l3_key].shape:
                    processed_dict[l1_l3_key] = v

            # 3. 處理獨立的 backbone_l4 (layer4)
            if k.startswith("layer4."):
                l4_key = f"backbone_l4.{k.replace('layer4.', '')}"
                if l4_key in model_dict:
                    if v.shape == model_dict[l4_key].shape:
                        processed_dict[l4_key] = v

        model_dict.update(processed_dict)
        self.load_state_dict(model_dict)
        print(f"✅ Grafted {len(processed_dict)} tensors. (Note: 3x3 grouped-convs remain randomly initialized)")
    ...
    def forward(self, x, return_attn=False):
        ...
        # 3. 雙表頭融合決策
        embed_cbp, embed_gem = self.embedding_cbp(p4_cbp), self.embedding_gem(torch.cat([p3, p4_gem], dim=1))
        logits_cbp, logits_gem = self.classifier_cbp(embed_cbp), self.classifier_gem(embed_gem)
